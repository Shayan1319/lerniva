...
$sql = "SELECT 
    ...
    COALESCE(
        ...
        IF(LOWER(m.sender_designation) = 'school', sch.logo, NULL)
    ) AS sender_photo
FROM messages m
...
LEFT JOIN schools sch 
    ON LOWER(m.sender_designation) = 'school' AND m.sender_id = sch.id
WHERE 
(
...
while ($row = mysqli_fetch_assoc($result)) {
    ...
    $imagePath = !empty($row['sender_photo']) ? 'uploads/profile/' . $row['sender_photo'] : 'assets/img/users/default.png';
    ...
    if (!empty($row['file_attachment'])) {
        $fileUrl = 'uploads/chat_files/' . $row['file_attachment'];
    ...
    if (!empty($row['voice_note'])) {
        $voiceUrl = 'uploads/voice_notes/' . $row['voice_note'];
        echo '<div style="margin-top: 5px;">
